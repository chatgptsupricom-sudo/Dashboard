  ...
  if (loading && !resumen) {
    return (
      <div className="flex flex-col items-center justify-center h-[80vh]">
        ...
        <p className="text-slate-500 text-sm mt-2">{t("loading_desc")}</p>
      </div>
    );
  }
    ...
    <div className="p-4 sm:p-8 space-y-8 bg-slate-50/30 min-h-screen">
      ...
      {r.enQuiebre > 0 && (
        <div className="flex items-center gap-3 bg-red-600 rounded-2xl px-4 py-3 text-white">
          ...
          <span className="text-sm font-medium">
            {t("quiebre_producto", { count: r.enQuiebre, s: r.enQuiebre > 1 ? "s" : "" })}
          </span>
          <Link href={`${base}/sugeridos`} className="ml-auto shrink-0">
            ...
            >
              {t("ver_ahora")} <ArrowRight className="h-3 w-3 ml-1" />
            </Button>
          </Link>
        </div>
      )}
      ...
      {r.capitalEstancado > 10000 && (
        <div className="flex items-center gap-3 bg-amber-500 rounded-2xl px-4 py-3 text-white">
          ...
          <span className="text-sm font-medium">
            {t("capital_inmovilizado_banner", { amount: fmt(r.capitalEstancado), count: r.totalEstancados })}
          </span>
          <Link href={`${base}/menor_rotacion`} className="ml-auto shrink-0">
            <Button
              ...
              className="bg-transparent border-white/40 text-white hover:bg-white/10 text-xs h-7 px-3"
            >
              {t("revisar")} <ArrowRight className="h-3 w-3 ml-1" />
            </Button>
          </Link>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <MetricCard
          title={t("valor_comprar")}
          value={`$${fmt(r.valorTotalComprar)}`}
          subtitle={t("skus_sugeridos", { count: r.totalSugeridos })}
          icon={ShoppingCart}
          color="blue"
          href={`${base}/sugeridos`}
        />
        ...
        <MetricCard
          ...
          subtitle={t("punto_reorden")}
          icon={TrendingDown}
          color="orange"
          href={`${base}/mayor_rotacion`}
        />
        <MetricCard
          title={t("capital_inmovilizado")}
          value={`$${fmt(r.capitalEstancado)}`}
          subtitle={t("productos_estancados", { count: r.totalEstancados })}
          icon={PackageSearch}
          color="slate"
        ...
        <Card className="rounded-3xl border-none shadow-sm bg-white overflow-hidden hover:shadow-md transition-shadow cursor-pointer">
          <CardHeader className="pb-3 border-b border-slate-50 flex flex-row items-center justify-between space-y-0">
            <CardTitle className="text-sm font-bold text-slate-700 flex items-center gap-2">
              <BarChart3 size={18} className="opacity-70" /> {t("clasificacion_abc")}
            </CardTitle>
            <span className="text-[10px] font-bold text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">
              {t("skus_activos", { count: r.totalSkusActivos })}
            </span>
          </CardHeader>
          <CardContent className="p-6">
            <div className="grid grid-cols-3 gap-4 mb-5">
              <div className="text-center">
                ...
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-0.5">{t("clase_a")}</p>
                <p className="text-[10px] text-emerald-600 font-bold mt-0.5">
                  {Math.round(pctA)}% · {t("top_ventas")}
                </p>
              </div>
              <div className="text-center border-x border-slate-100">
                ...
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-0.5">{t("clase_b")}</p>
                <p className="text-[10px] text-amber-600 font-bold mt-0.5">
                  {Math.round(pctB)}% · {t("media_rotacion")}
                </p>
              </div>
              <div className="text-center">
                ...
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-0.5">{t("clase_c")}</p>
                <p className="text-[10px] text-slate-400 font-bold mt-0.5">
                  {Math.round(pctC)}% · {t("baja_rotacion")}
                </p>
              </div>
            </div>
            ...
            <div className="mt-5 flex justify-end">
              ...
              >
                {t("ver_analisis")} <ArrowRight className="h-4 w-4 ml-2" />
              </Button>
            </div>
          </CardContent>
        </Card>
