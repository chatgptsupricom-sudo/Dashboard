...
//       { header: "Dirección", key: "street", width: 50 },
//     ];
...
//     });
//     saveAs(blob, `Cartera_Clientes${isPresentationMode ? "_Demo" : "_Supricom"}.xlsx`);
//   };
...
//           <ClientCard key={c.id} cliente={c} isPresentationMode={isPresentationMode} />
//         ))}
//       </div>
//     </div>
//   );
...
// function ClientCard({ cliente, isPresentationMode }: { cliente: any; isPresentationMode: boolean }) {
//   return (
...
//           <InfoRow icon={Phone} text={isPresentationMode ? "XXX-XXXXXXX" : (cliente.phone || "Sin teléfono")} />
//           <InfoRow icon={Mail} text={isPresentationMode ? "xxxxx@xxxxx.com" : (cliente.email || "Sin email")} />
//           <InfoRow icon={MapPin} text={isPresentationMode ? "[Dirección oculta]" : (cliente.street || "Sin dirección")} />
//         </div>
...
export default function MisClientesPage() {
  ...
  const exportarAExcel = async () => {
    ...
    // 1. Agregamos las nuevas columnas con sus anchos correspondientes
    worksheet.columns = [
      { header: "Nombre Cliente", key: "name", width: 35 },
      { header: "RIF/Identificación", key: "vat", width: 20 },
      { header: "Teléfono", key: "phone", width: 20 },
    ...
    clientesFiltrados.forEach((c: any) => {
      // Nota: Verifica cómo se llaman exactamente estas llaves en tu API backend
      // (ej. c.payment_terms, c.credit_limit, c.balance, etc.) y cámbialas si es necesario.
      worksheet.addRow({
        name: isPresentationMode ? "Cliente de ejemplo" : c.name,
        vat: isPresentationMode ? "J-XXXXXXXX-X" : c.vat || "N/A",
        phone: isPresentationMode ? "XXX-XXXXXXX" : c.phone || "N/A",
    ...
    // Formatear celdas numéricas y alinear columnas monetarias/de saldos
    worksheet.eachRow((row, rowNumber) => {
      row.eachCell((cell, colNumber) => {
        ...
        // Si no es el encabezado, dar formato monetario/decimal a las columnas de Límite de Crédito (Col 7) y Saldo (Col 8)
        if (rowNumber > 1) {
