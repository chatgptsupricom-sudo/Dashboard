...
function getMonthOptions(locale: string, count: number = 6) {
  const now = new Date();
...
export default function VendedoresPage() {
  ...
  const [monthOptions, setMonthOptions] = useState(() => getMonthOptions(locale, 6));
  ...
  return (
    <div className="space-y-4 sm:space-y-6 bg-slate-50/50 p-4 sm:p-8 min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4 sm:mb-6">
        ...
        >
          <option value="total">{t("historical_total")}</option>
      ...
      >
        <div className="flex items-center justify-between">
          ...
          {cuota && (
            <span className="text-[10px] sm:text-xs text-slate-400">{t("click_detalle")}</span>
          )}
        </div>
        {cuota ? (
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-1 sm:gap-2">
              <div className="min-w-0">
                ...
                <span className="text-[10px] sm:text-sm text-slate-400 ml-1 sm:ml-2 block sm:inline">
                  {t("de")} ${(cuota.meta || 0).toLocaleString()}
                </span>
              </div>
              <span
            ...
            <div className="flex items-center justify-between text-[10px] sm:text-sm gap-2">
              {cuota.falta > 0 && (
                <span className="text-red-500 font-semibold truncate">
                  {t("faltan", { amount: cuota.falta.toLocaleString() })}
                </span>
              )}
              {cuota.porcentaje >= 100 && (
                <span className="text-emerald-600 font-bold flex items-center gap-1">
                  <TrendingUp size={14} /> {t("meta_cumplida")}
                </span>
              )}
            ...
            <div className="bg-slate-50 p-3 sm:p-4 rounded-xl border border-slate-100 space-y-3">
              <div className="grid grid-cols-2 gap-y-2 text-xs sm:text-sm">
                ...
                <span className="text-slate-500">
                  {t("facturado_al", { date: new Date().toLocaleDateString() })}
                </span>
                <span className="font-mono font-bold text-right">
                  ${(cuota.facturado || 0).toLocaleString()}
                </span>
                {cuota.falta > 0 ? (
                  ...
                  <>
                    <span className="text-slate-500">{t("excedente")}</span>
                    ...
                    {cuota.falta > 0 && m.diasHabilesRestantes > 0 && (
                      <div className="bg-blue-50 border border-blue-200 rounded-lg p-2.5 sm:p-3">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-xs font-bold text-blue-800">{t("dias_habiles_restantes", { count: m.diasHabilesRestantes })}</span>
                        </div>
                        <p
                          className="text-xs text-blue-700"
                          dangerouslySetInnerHTML={{ __html: t("venta_diaria_necesaria", { amount: m.ventaDiariaNecesaria.toLocaleString() }) }}
                        />
                      </div>
                    )}
                    <div className={`border rounded-lg p-2.5 sm:p-3 ${supero150 ? "bg-emerald-50 border-emerald-200" : "bg-purple-50 border-purple-200"}`}>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className={`text-xs font-bold ${supero150 ? "text-emerald-800" : "text-purple-800"}`}>
                          {supero150 ? t("supero_150") : t("para_llegar_150")}
                        </span>
                      </div>
                      <p
                        className={`text-xs ${supero150 ? "text-emerald-700" : "text-purple-700"}`}
                        dangerouslySetInnerHTML={{
                          __html: supero150
                            ? t("supero_150_por", { amount: ((cuota.facturado || 0) - m.meta150).toLocaleString() })
                            : t("faltan_para_150", { amount: m.faltaPara150.toLocaleString(), target: m.meta150.toLocaleString() })
                        }}
                      />
                    </div>
                  </>
              ...
              {cuota.falta > 0 ? (
                <div className="bg-red-50 border border-red-200 rounded-lg p-2.5 sm:p-3 text-center">
                  <p className="text-xs sm:text-sm font-bold text-red-700">
                    {t("faltan_cuota", { amount: cuota.falta.toLocaleString() })}
                  </p>
            ...
            <div>
              ...
              <p className="text-[10px] sm:text-xs text-slate-400 font-medium mt-0.5">
                {/^\d{4}-\d{2}$/.test(periodo)
                  ? t("ventas_por_dia", { period: monthOptions.find((o) => o.value === periodo)?.label || periodo })
                  : t("ventas_mensuales")}
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-6">
          <Card className="rounded-2xl sm:rounded-3xl border-none shadow-sm bg-white p-4 sm:p-6 flex flex-col justify-center">
            ...
            <p className="text-[8px] sm:text-[10px] text-emerald-600 font-bold mt-1 sm:mt-2">
              {isPresentationMode
                ? "Total: $ XX,XXX USD"
                : t("total_usd", { amount: (data.montoLeads || 0).toLocaleString() })}
            </p>
          </Card>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-6">
        <RankList
          title={t("top_5_clientes")}
          data={data.topClients}
          isPresentationMode={isPresentationMode}
        />
        <RankList
          title={t("top_5_productos")}
