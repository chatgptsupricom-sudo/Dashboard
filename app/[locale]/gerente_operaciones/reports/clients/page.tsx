...
export default function ClientsReportComponent() {
  ...
  const companies = userCids
    ? ALL_COMPANIES.filter((c) => c.id === String(userCids))
    : ALL_COMPANIES;

  const [selectedCompany, setSelectedCompany] = useState(
    userCids ? String(userCids) : "all",
  );
  ...
  function resetFilters() {
    setSelectedCompany(userCids ? String(userCids) : "all");
    setStartDate("");
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex flex-col xl:flex-row items-start xl:items-center justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full xl:w-auto">
          <select
            ...
            disabled={!!userCids}
            className="w-full sm:w-48 h-10 px-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all cursor-pointer shadow-sm disabled:opacity-70 disabled:cursor-default"
          >
            {!userCids && <option value="all">🏢 Todas las Empresas</option>}
            {companies.map((c) => (
