...
// Reporte KPI por campana, en paginas imprimibles. Mismo patron que el informe
// mensual: vive bajo /adminleads para heredar el guard de rol del middleware,
// se monta como overlay sobre el layout y al imprimir oculta todo lo demas.

import ReporteCampanas from "@/components/leads/ReporteCampanas";
...
function ReporteLoader() {
  ...
  const margen = params.get("margen") || "";
  ...
  useEffect(() => {
    ...
    if (margen) qs.set("margen", margen);

    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 90000);

    fetch(`/api/adminleads/reporte-campanas?${qs}`, { signal: controller.signal })
      .then((r) => r.json())
      .then((r) => {
  ...
  }, [fechaInicio, fechaFin, sede, margen]);
  ...
  return <ReporteCampanas data={data} />;
}
