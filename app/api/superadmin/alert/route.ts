  ...
  try {
    ...
    const [
      ...
      callOdooRPC(
        ...
        { fields: ["invoice_user_id", "user_id"] },
      ),
    ]);
    ...
    // Quienes ya facturaron hoy (query dedicada con filtro Odoo)
    const facturasHoy = Array.isArray(facturasHoyRaw) ? facturasHoyRaw : [];
    const vendedoresActivosHoyIds = new Set<number>();
    const vendedoresActivosHoyNames = new Set<string>();
    for (const f of facturasHoy) {
      if (f.invoice_user_id && Array.isArray(f.invoice_user_id)) {
        vendedoresActivosHoyIds.add(Number(f.invoice_user_id[0]));
        vendedoresActivosHoyNames.add(
          String(f.invoice_user_id[1] || "").toUpperCase().trim(),
        );
      }
      if (f.user_id && Array.isArray(f.user_id)) {
        vendedoresActivosHoyIds.add(Number(f.user_id[0]));
        vendedoresActivosHoyNames.add(
          String(f.user_id[1] || "").toUpperCase().trim(),
        );
      }
    }
    ...
    try {
      ...
      if (!isWeekend) {
        vendedoresInactivos = allSellers
          ...
          .filter(
            (v: any) =>
              !vendedoresActivosHoyIds.has(Number(v.user_id)) &&
              !vendedoresActivosHoyNames.has(v.name.toUpperCase().trim()) &&
    ...
    const vendedoresActivosHoyCount = vendedoresActivosHoyIds.size;
