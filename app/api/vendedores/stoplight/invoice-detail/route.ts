...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!invoiceId) return NextResponse.json({ error: "Falta invoice_id" }, { status: 400 });

    const invoiceData = await callOdooRPC<any[]>(
      "account.move", "search_read",
      [[["id", "=", parseInt(invoiceId, 10)]]],
      { fields: ["id", "name", "invoice_date", "amount_untaxed", "amount_tax", "amount_total", "move_type", "partner_id", "invoice_user_id"], limit: 1 }
    );
    ...
    const lines = await callOdooRPC<any[]>(
      ...
      { fields: ["product_id", "name", "quantity", "price_unit", "price_subtotal", "price_total"], limit: 100 }
    );
    ...
    console.error("Error en API invoice-detail vendedor:", error.message);
