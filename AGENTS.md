...
- Gestor de paquetes: **pnpm** (Dockerfile y docs usan pnpm; hay `package-lock.json` y `pnpm-lock.yaml` duplicados, ignorar el de npm).
...
- Roles/permisos se comparan en minúsculas en `middleware.ts`. Rutas protegidas: `/dashboard`, `/superadmin`, `/vendedores`, `/adminleads`, `/gerente_venta`, `/gerente_operaciones`, `/recursos_humanos`, `/compras`. Al añadir un rol, hay que tocarlo en middleware, `lib/types.ts` y `lib/actividades/rolesConfig.ts`.
...
- Varias rutas (inventario, agenteia) hacen `process.env.NODE_TLS_REJECT_UNAUTHORIZED = "0"` al inicio — quirk de dev, respetarlo.
